...
int AudioEngine::process(const float*in,float*out,unsigned long frames){
    ...
    for(unsigned long i=0;i<frames;++i){float&l=out[i*2];float&r=out[i*2+1];if(g)for(auto const&pl:g->masterPlugins)processRealtimeBuiltinSample(pl,l,r);l=std::clamp(l,-1.0f,1.0f);r=std::clamp(r,-1.0f,1.0f);}if(transport)playhead_.store(base+static_cast<SampleIndex>(frames),std::memory_order_release);return 0;
}
