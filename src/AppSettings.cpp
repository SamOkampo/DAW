...
void saveAppSettings(const AppSettings&s,const std::filesystem::path&path){
    ...
    f<<"SAMPLE_ROOTS "<<std::min<std::size_t>(s.sampleRoots.size(),64)<<"\n";
    for(std::size_t i=0;i<s.sampleRoots.size()&&i<64;++i)f<<"SAMPLE_ROOT "<<std::quoted(s.sampleRoots[i].string())<<"\n";
    f<<"FAVORITE_SAMPLES "<<std::min<std::size_t>(s.favoriteSamples.size(),256)<<"\n";
    for(std::size_t i=0;i<s.favoriteSamples.size()&&i<256;++i)f<<"FAVORITE_SAMPLE "<<std::quoted(s.favoriteSamples[i].string())<<"\n";
    f<<"RECENT_SAMPLES "<<std::min<std::size_t>(s.recentSamples.size(),64)<<"\n";
    for(std::size_t i=0;i<s.recentSamples.size()&&i<64;++i)f<<"RECENT_SAMPLE "<<std::quoted(s.recentSamples[i].string())<<"\n";
    f<<"END\n";}
...
AppSettings loadAppSettings(const std::filesystem::path&path){
    auto s=defaultAppSettings();std::ifstream f(path);if(!f)return s;std::string tag;int version=0;f>>tag>>version;if(tag!="FLOWDAW_SETTINGS"||(version<1||version>3))return s;
    s.version=3;s.pluginRoots.clear();s.sampleRoots.clear();s.favoriteSamples.clear();s.recentSamples.clear();
    while(f>>tag){if(tag=="SAMPLE_RATE")f>>s.audio.preferredSampleRate;else if(tag=="BUFFER")f>>s.audio.bufferSize;else if(tag=="INPUT")f>>std::quoted(s.audio.inputDevice);else if(tag=="OUTPUT")f>>std::quoted(s.audio.outputDevice);else if(tag=="MONITOR"){int v;f>>v;s.audio.monitorByDefault=v!=0;}else if(tag=="AUTOSAVE")f>>s.autosaveSeconds;else if(tag=="RESTORE"){int v;f>>v;s.restoreLastSession=v!=0;}else if(tag=="LAST_PROJECT"){std::string v;f>>std::quoted(v);s.lastProjectPath=v;}else if(tag=="PLUGIN_ROOTS"){std::size_t n;f>>n;}else if(tag=="PLUGIN_ROOT"){std::string v;f>>std::quoted(v);s.pluginRoots.emplace_back(v);}else if(tag=="SAMPLE_ROOTS"){std::size_t n;f>>n;}else if(tag=="SAMPLE_ROOT"){std::string v;f>>std::quoted(v);if(s.sampleRoots.size()<64)s.sampleRoots.emplace_back(v);}else if(tag=="FAVORITE_SAMPLES"){std::size_t n;f>>n;}else if(tag=="FAVORITE_SAMPLE"){std::string v;f>>std::quoted(v);if(s.favoriteSamples.size()<256)s.favoriteSamples.emplace_back(v);}else if(tag=="RECENT_SAMPLES"){std::size_t n;f>>n;}else if(tag=="RECENT_SAMPLE"){std::string v;f>>std::quoted(v);if(s.recentSamples.size()<64)s.recentSamples.emplace_back(v);}else if(tag=="END")break;else{std::string ignored;std::getline(f,ignored);}}
