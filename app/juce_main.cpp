...
public:
    MainComponent(){
        ...
        trackChoice_.onChange=[this]{refreshMixerTargets();syncMixerControls();refreshRackControls();};
        ...
        mixerTargetChoice_.setTextWhenNothingSelected("No mixer target");mixerTargetChoice_.onChange=[this]{if(!suppressMixerCallbacks_)syncMixerControls();};addAndMakeVisible(mixerTargetChoice_);
        ...
        refreshTrackChoice();refreshMixerTargets();refreshPatternChoice();refreshSampleChoice();syncMixerControls();refreshRackControls();syncChopControls();updateBpmLabel();setEditorMode(EditorMode::Arrangement);
