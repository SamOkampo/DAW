...
public:
    MainComponent(){
        ...
        const auto config=defaultSettingsDirectory();settingsPath_=config/"settings.conf";safetyPath_=config/"plugin-safety.conf";recovery_=std::make_unique<SessionRecovery>(config/"recovery");settings_=std::filesystem::exists(settingsPath_)?loadAppSettings(settingsPath_):defaultAppSettings();safety_.load(safetyPath_);
        const auto recoveryInfo=recovery_->inspect();bool loadedRecovery=false,loadedLast=false;
        if(settings_.restoreLastSession&&recoveryInfo.available)try{project_=recovery_->loadRecovered(true);projectPath_=recoveryInfo.originalProject;loadedRecovery=true;recoveredAtStartup_=true;}catch(...){}
        ...
        status_.setText(recoveredAtStartup_?"Recovered previous autosave; save when ready":(err.isEmpty()?"JUCE AudioDeviceManager drives FLOWDAW core":"Audio: "+err),juce::dontSendNotification);addAndMakeVisible(status_);
        ...
        note_.setText("JUCE is the production Studio. Phase 9 focuses on workflow, browser, autosave/recovery and product polish while file I/O stays off the audio callback.",juce::dontSendNotification);note_.setJustificationType(juce::Justification::centredLeft);addAndMakeVisible(note_);
    ...
    ~MainComponent()override{if(audioRecording_)finishAudioRecording();if(recordingChops_)finishChopRecording();pluginWindow_.reset();deviceManager_.removeAudioCallback(this);engine_.stop();engine_.collectRetiredGraphs();saveDeviceSettings();saveSafety();if(recovery_)try{recovery_->markCleanExit();}catch(...){}}
...
private:
    ...
    void newProject(){
        ...
        projectLabel_.setText("Untitled Beat • starter project",juce::dontSendNotification);status_.setText("New project: drums + FLOW Keys ready",juce::dontSendNotification);resetRecoverySnapshot();saveDeviceSettings();
    }
