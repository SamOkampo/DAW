...
public:
    MainComponent(){
        ...
        sampleBrowser_=std::make_unique<juceui::SampleBrowserComponent>(settings_,[this](const std::filesystem::path&path){importWavFile(path);},[this]{saveDeviceSettings();});addAndMakeVisible(*sampleBrowser_);
    ...
    void resized()override{
        ...
        r.removeFromTop(4);auto workspace=r;auto browserArea=workspace.removeFromLeft(286);if(sampleBrowser_)sampleBrowser_->setBounds(browserArea.reduced(3));workspace.removeFromLeft(6);
        auto editorArea=workspace.removeFromTop(320);if(arrangement_)arrangement_->setBounds(editorArea);if(piano_)piano_->setBounds(editorArea);if(sampler_)sampler_->setBounds(editorArea);if(sequencer_)sequencer_->setBounds(editorArea);if(automationAssist_)automationAssist_->setBounds(editorArea);
        auto mixer=workspace.removeFromTop(118);mixerVolumeLabel_.setBounds(mixer.removeFromLeft(95).reduced(3));mixerVolume_.setBounds(mixer.removeFromLeft(245).reduced(3));mixerPanLabel_.setBounds(mixer.removeFromLeft(45).reduced(3));mixerPan_.setBounds(mixer.removeFromLeft(220).reduced(3));muteTrack_.setBounds(mixer.removeFromLeft(70).reduced(3));soloTrack_.setBounds(mixer.removeFromLeft(70).reduced(3));trackMeterLabel_.setBounds(mixer.removeFromLeft(120).reduced(3));trackMeter_.setBounds(mixer.reduced(3));
        workspace.removeFromTop(6);selector_->setBounds(workspace);
    }
private:
