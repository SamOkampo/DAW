...
public:
    ...
    void resized()override{
        ...
        auto rack=r.removeFromTop(38);rackTargetChoice_.setBounds(rack.removeFromLeft(155).reduced(3));rackPluginChoice_.setBounds(rack.removeFromLeft(225).reduced(3));addRackGain_.setBounds(rack.removeFromLeft(98).reduced(3));addRackClip_.setBounds(rack.removeFromLeft(90).reduced(3));addRackWidth_.setBounds(rack.removeFromLeft(80).reduced(3));addRackExternal_.setBounds(rack.removeFromLeft(86).reduced(3));rackMoveUp_.setBounds(rack.removeFromLeft(45).reduced(3));rackMoveDown_.setBounds(rack.removeFromLeft(50).reduced(3));rackBypass_.setBounds(rack.removeFromLeft(72).reduced(3));rackRemove_.setBounds(rack.removeFromLeft(72).reduced(3));openRackEditor_.setBounds(rack.removeFromLeft(100).reduced(3));rackWet_.setBounds(rack.removeFromLeft(145).reduced(3));rackParamLabel_.setBounds(rack.removeFromLeft(68).reduced(3));rackParam_.setBounds(rack.reduced(3));
...
private:
    ...
    void refreshRackControls(){
        suppressRackCallbacks_=true;refreshRackTargets();const int previous=rackPluginChoice_.getSelectedId();rackPluginChoice_.clear(juce::dontSendNotification);auto*rack=rackPlugins();if(rack){int id=1;for(auto const&p:*rack){auto label=juce::String(p.name)+" ["+juce::String(p.format)+"] "+(p.bypass?"BYPASS":"ACTIVE");rackPluginChoice_.addItem(label,id++);}if(!rack->empty())rackPluginChoice_.setSelectedId(previous>0&&previous<=static_cast<int>(rack->size())?previous:1,juce::dontSendNotification);}suppressRackCallbacks_=false;syncRackControls();
    }
    void syncRackControls(){
        suppressRackCallbacks_=true;auto*p=selectedRackPlugin();auto*rack=rackPlugins();const int index=rackPluginChoice_.getSelectedId()-1;rackWet_.setEnabled(p!=nullptr);rackBypass_.setEnabled(p!=nullptr);rackRemove_.setEnabled(p!=nullptr);rackMoveUp_.setEnabled(p&&index>0);rackMoveDown_.setEnabled(p&&rack&&index+1<static_cast<int>(rack->size()));addRackExternal_.setEnabled(rack!=nullptr);openRackEditor_.setEnabled(p&&p->format!="builtin");
