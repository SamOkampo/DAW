...
public:
    MainComponent(){
        ...
        note_.setText("JUCE now owns device I/O and drives the FLOWDAW engine. Real VST3/AU loading/editor hosting is active; project insert integration remains a separate block-processing migration, so X11 stays available for full editing views.",juce::dontSendNotification);note_.setJustificationType(juce::Justification::centredLeft);addAndMakeVisible(note_);
        setSize(1100,760);startTimer(5000);
    }
    ~MainComponent()override{deviceManager_.removeAudioCallback(this);engine_.stop();saveDeviceSettings();}
...
private:
    void audioDeviceAboutToStart(juce::AudioIODevice*device)override{if(!device)return;engine_.configureExternalDevice(static_cast<int>(device->getCurrentSampleRate()),static_cast<unsigned long>(device->getCurrentBufferSizeSamples()),!device->getActiveInputChannels().isZero());engine_.publish(project_);}
