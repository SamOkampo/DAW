...
static void testPreparedChain(){
    RealtimePluginChain missingHostChain;std::vector<RealtimePluginIssue>missingHostIssues;require(!missingHostChain.prepare({fakePlugin("missing.backend")},nullptr,48000,2,8,&missingHostIssues),"external plugin without a host must fail graph preparation");require(missingHostIssues.size()==1&&missingHostIssues[0].message.find("No external plugin host")!=std::string::npos,"missing plugin host must surface a useful preparation issue");
    PluginHost missingPluginHost;missingPluginHost.registerBackend(std::make_shared<FakeMidiBackend>());RealtimePluginChain missingPluginChain;std::vector<RealtimePluginIssue>missingPluginIssues;require(!missingPluginChain.prepare({fakePlugin("missing.plugin")},&missingPluginHost,48000,2,8,&missingPluginIssues),"persisted plugin that is no longer available must fail graph preparation");require(missingPluginIssues.size()==1&&missingPluginIssues[0].message.find("Unknown fake instrument")!=std::string::npos,"missing plugin must surface the backend creation error");
