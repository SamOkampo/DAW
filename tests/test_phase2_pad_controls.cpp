...
int main(){
    try{
        ...
        const auto path=std::filesystem::temp_directory_path()/"flowdaw_phase2_v7.flow";
        ProjectSerializer::save(p,path);auto loaded=ProjectSerializer::load(path,false);
        require(loaded.formatVersion==7,"project must migrate/save as v7");
        require(loaded.samples.size()==1&&loaded.samples[0].slices.size()==2,"v7 slices must survive save/load");
        ...
        require(legacy.formatVersion==7,"v6 project must migrate to v7 in memory");
