...
int main(){
 try{
    ...
    const auto projectPath=std::filesystem::temp_directory_path()/"flowdaw_phase4_v9.flow";ProjectSerializer::save(p,projectPath);auto loaded=ProjectSerializer::load(projectPath,false);require(loaded.formatVersion==9,"project must save/load as v9");require(loaded.buses.size()==1&&loaded.tracks[0].sends.size()==1,"buses and sends persist");require(loaded.tracks[0].takes.size()==1&&loaded.tracks[0].activeTakeId==take.id,"recording takes and comp selection persist");require(loaded.automation.size()==1&&loaded.automation[0].target=="send.gain","automation lanes persist");std::filesystem::remove(projectPath);

    const auto legacyPath=std::filesystem::temp_directory_path()/"flowdaw_phase4_legacy_v8.flow";{std::ofstream f(legacyPath);f<<"FLOWDAW_PROJECT 8\nNAME \"legacy\"\nSAMPLE_RATE 48000\nBPM 90\nPLAYHEAD 0\nMASTER 1 0\nSAMPLES 0\nTRACKS 0\nPATTERNS 0\nEND\n";}auto legacy=ProjectSerializer::load(legacyPath,false);std::filesystem::remove(legacyPath);require(legacy.formatVersion==9,"v8 projects migrate to v9");
