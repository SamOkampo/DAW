...
int main(){
    try{
        ...
        Project longSeq=seq;auto& longPat=longSeq.patterns[0];longPat.stepCount=64;for(auto& lane:longPat.lanes){lane.steps.resize(64);for(auto& st:lane.steps)st.active=false;}longPat.lanes[0].steps[63].active=true;longPat.lanes[0].steps[63].velocity=0.77f;AudioEngine longEngine;longEngine.publish(longSeq);auto longAudio=longEngine.renderOffline(MusicalTime::ticksToSamples(longPat.lengthTicks(),90,48000));require(firstAudibleFrame(longAudio)>500000,"64-step pattern schedules final page");const auto longPath=std::filesystem::temp_directory_path()/"flowdaw_64.flow";ProjectSerializer::save(longSeq,longPath);auto longLoaded=ProjectSerializer::load(longPath);require(longLoaded.patterns[0].stepCount==64&&longLoaded.patterns[0].lanes[0].steps.size()==64,"64-step length persists");require(longLoaded.patterns[0].lanes[0].steps[63].active,"64-step final event persists");

        Project movedSeq=seq;movedSeq.tracks[0].patternClips[0].startTick=4*kPPQ;AudioEngine movedEngine;movedEngine.publish(movedSeq);auto movedAudio=movedEngine.renderOffline(180000);require(firstAudibleFrame(movedAudio)>=128000,"arrangement pattern start moves rendered audio");const auto movedPath=std::filesystem::temp_directory_path()/"flowdaw_moved_pattern.flow";ProjectSerializer::save(movedSeq,movedPath);auto movedLoaded=ProjectSerializer::load(movedPath);require(movedLoaded.tracks[0].patternClips[0].startTick==4*kPPQ,"pattern arrangement position persists");
