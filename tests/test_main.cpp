...
try{
    ...
    const auto tmp=std::filesystem::temp_directory_path()/"flowdaw_test.wav";auto a=tone();WavFile::writeFloat32(tmp,a);auto b=WavFile::read(tmp);require(b.sampleRate==48000&&b.channels==1&&b.frames()==a.frames(),"wav roundtrip");auto peaks=buildWaveform(b,100);require(peaks.size()==100,"waveform");

    Project p;p.name="Round Trip Beat";p.transport.bpm=90;SampleAsset s;s.name="tone";s.path=tmp;s.detectedBpm=91.25;s.bpmConfidence=.81f;s.audio=std::make_shared<AudioBuffer>(b);SampleSlice sl;sl.name="Slice 1";sl.startFrame=100;sl.endFrame=1000;s.slices.push_back(sl);auto sid=s.id;p.samples.push_back(s);Track tr;tr.name="Sample";Clip c;c.sampleId=sid;c.sourceLength=b.frames();c.lengthTicks=MusicalTime::samplesToTicks(b.frames(),90,48000);tr.clips.push_back(c);Effect fx;fx.type="gain";fx.value=.8f;tr.mixer.effects.push_back(fx);p.tracks.push_back(tr);
    const auto proj=std::filesystem::temp_directory_path()/"flowdaw_test.flow";ProjectSerializer::save(p,proj);auto loaded=ProjectSerializer::load(proj);require(loaded.formatVersion==5&&loaded.samples[0].slices.size()==1,"v5 slice project roundtrip");require(std::abs(loaded.samples[0].detectedBpm-91.25)<.01&&std::abs(loaded.samples[0].bpmConfidence-.81f)<.01f,"BPM metadata persists");require(loaded.tracks[0].mixer.effects.size()==1,"effects persist");
    ...
    p=before;AudioEngine engine;engine.publish(p);auto rendered=engine.renderOffline(48000);require(energy(rendered)>10.0,"offline engine renders audible signal");
    ...
    auto seq=makeSequencer(64);auto&pat=seq.patterns[0];pat.lanes[0].steps[0].active=true;pat.lanes[0].steps[63].active=true;pat.lanes[1].steps[4].active=true;for(int i=0;i<64;i+=2)pat.lanes[2].steps[static_cast<std::size_t>(i)].active=true;seq.tracks[0].patternClips[0].repeats=1;AudioEngine seqEngine;seqEngine.publish(seq);auto seqAudio=seqEngine.renderOffline(MusicalTime::ticksToSamples(pat.lengthTicks(),90,48000));require(energy(seqAudio)>50.0,"64-step sequencer renders");
    const auto seqPath=std::filesystem::temp_directory_path()/"flowdaw_seq.flow";ProjectSerializer::save(seq,seqPath);auto seqLoaded=ProjectSerializer::load(seqPath);require(seqLoaded.patterns[0].stepCount==64&&seqLoaded.patterns[0].lanes[0].steps[63].active,"step 64 persists");

    auto straight=makeSequencer();straight.patterns[0].lanes[0].steps[1].active=true;AudioEngine straightEngine;straightEngine.publish(straight);auto straightAudio=straightEngine.renderOffline(30000);auto straightOnset=firstAudible(straightAudio);
    auto swung=straight;swung.patterns[0].swing=1.0f;AudioEngine swingEngine;swingEngine.publish(swung);auto swingAudio=swingEngine.renderOffline(30000);require(firstAudible(swingAudio)>straightOnset+3000,"swing affects actual scheduling");
    swung.patterns[0].lanes[0].steps[1].probability=0.0f;swingEngine.publish(swung);require(energy(swingEngine.renderOffline(30000))<0.01,"probability zero suppresses hit");
    swung.patterns[0].lanes[0].steps[1].probability=1.0f;swung.patterns[0].lanes[0].mute=true;swingEngine.publish(swung);require(energy(swingEngine.renderOffline(30000))<0.01,"lane mute suppresses hit");
    ...
    auto sinus=tone(48000,2.0,220);auto shortTone=timeStretchWsola(sinus,.8);require(std::llabs(shortTone.frames()-static_cast<SampleIndex>(sinus.frames()*.8))<=1,"WSOLA duration");require(std::abs(estimateFrequency(shortTone,5000,30000)-220)<8,"WSOLA approximately preserves pitch");
    auto clicks82=clickTrack(82,28);auto matched90=matchBpmWsola(clicks82,82,90);auto matchedEstimate=detectBpm(matched90,70,120);require(matchedEstimate.valid()&&std::abs(matchedEstimate.bpm-90)<3.0,"82 BPM matches to 90 BPM");
    ...
    auto segmented=std::make_shared<AudioBuffer>();segmented->sampleRate=48000;segmented->channels=1;segmented->interleaved.assign(400,0.0f);std::fill(segmented->interleaved.begin()+100,segmented->interleaved.begin()+200,0.8f);std::fill(segmented->interleaved.begin()+200,segmented->interleaved.begin()+300,-0.5f);
    AudioEngine preview;require(preview.triggerPreview(segmented,100,100),"queue slice preview");auto previewBlock=preview.renderDeviceBlockForTest(160);require(energy(previewBlock)>40.0,"slice preview audible while transport stopped");double tail=0;for(SampleIndex f=110;f<160;++f)tail+=std::abs(previewBlock.interleaved[static_cast<std::size_t>(f*2)]);require(tail<0.001,"preview stops at slice boundary");
    require(preview.triggerPreview(segmented,200,100),"queue second slice");auto second=preview.renderDeviceBlockForTest(80);require(second.interleaved[0]<-0.2f,"second pad starts from its own slice");

    Project chops;chops.transport.bpm=90;SampleAsset chopSample;chopSample.name="Chop source";chopSample.audio=segmented;SampleSlice sliceA;sliceA.name="A";sliceA.startFrame=100;sliceA.endFrame=200;SampleSlice sliceB;sliceB.name="B";sliceB.startFrame=200;sliceB.endFrame=300;const auto sliceAId=sliceA.id,sliceBId=sliceB.id;chopSample.slices={sliceA,sliceB};const auto chopSampleId=chopSample.id;chops.samples.push_back(chopSample);Pattern chopPattern;chopPattern.name="Recorded Chops";ChopEvent hitA;hitA.tick=0;hitA.sampleId=chopSampleId;hitA.sliceId=sliceAId;ChopEvent hitB;hitB.tick=kPPQ;hitB.sampleId=chopSampleId;hitB.sliceId=sliceBId;chopPattern.chopEvents={hitA,hitB};const auto chopPatternId=chopPattern.id;chops.patterns.push_back(chopPattern);Track chopTrack;chopTrack.name="CHOPS";PatternPlacement chopPlacement;chopPlacement.patternId=chopPatternId;chopTrack.patternClips.push_back(chopPlacement);chops.tracks.push_back(chopTrack);AudioEngine chopEngine;chopEngine.publish(chops);const auto beatSamples=MusicalTime::ticksToSamples(kPPQ,90,48000);auto chopRender=chopEngine.renderOffline(beatSamples+600);require(chopRender.interleaved[0]>0.2f,"first recorded chop resolves first slice");require(chopRender.interleaved[static_cast<std::size_t>(beatSamples*2)]<-0.1f,"second recorded chop resolves second slice at beat 2");double gapEnergy=0;for(SampleIndex f=1000;f<beatSamples-1000;++f)gapEnergy+=std::abs(chopRender.interleaved[static_cast<std::size_t>(f*2)]);require(gapEnergy<0.001,"recorded chops stop at persisted slice boundaries");const auto chopPath=std::filesystem::temp_directory_path()/"flowdaw_chops_v5.flow";ProjectSerializer::save(chops,chopPath);auto chopsLoaded=ProjectSerializer::load(chopPath,false);require(chopsLoaded.formatVersion==5&&chopsLoaded.patterns[0].chopEvents.size()==2,"v5 ChopEvents persist");require(chopsLoaded.patterns[0].chopEvents[1].tick==kPPQ&&chopsLoaded.patterns[0].chopEvents[1].sliceId==sliceBId,"recorded chop timing and slice identity persist");

    std::cout<<"FLOWDAW Phase 0/1/2 + REC CHOPS v5 tests: PASS\n";return 0;
